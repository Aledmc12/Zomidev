...
import { BarChart3, FolderKanban, Users, LogOut, Menu, X } from 'lucide-react'
...
export default function AdminShell({ children }) {
  ...
  if (loading) {
    return <div className="flex min-h-screen items-center justify-center bg-bg text-muted">Cargando panel...</div>
  }
  ...
  return (
    <div className="flex min-h-screen bg-bg">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 border-r border-white/5 bg-bg-secondary transition lg:static ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
        aria-hidden={!mobileOpen && typeof window !== 'undefined' && window.innerWidth < 1024 ? true : undefined}
      >
        <div className="flex items-center justify-between border-b border-white/5 p-6">
          <Link href="/admin" className="flex items-center gap-3">
            <Logo size={32} />
            <span className="font-serif tracking-[0.15em]">Admin</span>
          </Link>
      ...
      <div className="flex min-h-screen flex-1 flex-col">
        <header className="flex items-center border-b border-white/5 px-6 py-4 lg:px-10">
          <button
            className="lg:hidden"
          ...
          <p className="ml-0 text-sm text-muted lg:ml-0">Panel administrativo ZomiDev</p>
        </header>
